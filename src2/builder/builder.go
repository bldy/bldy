...
func New(ctx build.Context, rt build.Runtime, c *Config, g *graph.Graph, n Notifier) (b Builder) {
	b.pq = pqueue.New()
	b.graph = g
	b.notifier = n
	b.rt = rt
...
func (b *Builder) install(job *graph.Node) error {
	if err := os.MkdirAll(
		*b.config.BuildOut,
		...
		l.Fatalf("copying job %s failed: %s", job.Task.Name(), err.Error())
	}
	...
	for _, output := range job.Task.Outputs() {
		...
		if err := os.MkdirAll(
			...
			l.Fatalf("linking job %s failed: %s", job.Task.Name(), err.Error())
		}
		...
		if os.IsNotExist(err) {
			return fmt.Errorf("cannot install %s: file %s doesn't exist", job.Task.Name(), src)
		}
		in, err := os.Open(d)
		if err != nil {
			l.Fatalf("copy: can't finiliaze %s. copying %q to %q failed: %s\n", job.Task.Name(), src, dstp, err)
		}
...
func (b *Builder) bindChildOutputs(n *graph.Node, ns namespace.Namespace) error {
	for _, c := range n.Children {
		for _, output := range c.Task.Outputs() {
