...
const assert = `# Predeclared built-ins for this module:
#
...
func isPredeclared(s string) bool { return true }
func init() {
	_, _, err := starlark.SourceProgram("assert.bldy", assert, isPredeclared)
	if err != nil {
		panic(err)
	}
}